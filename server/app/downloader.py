...
def _run_ytdlp(video_id: str) -> Path:
    """Bloquant — à exécuter dans un thread (run_in_executor)."""
...
async def prepare_video(video_id: str):
    ...
    await _set_status(video_id, state=STATUS_DOWNLOADING, ts=str(time.time()))
    try:
        loop = asyncio.get_running_loop()
        path = await loop.run_in_executor(None, _run_ytdlp, video_id)
